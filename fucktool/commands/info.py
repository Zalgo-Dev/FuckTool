# fucktool/commands/info.py

import requests
from core.colors import (
    RED, LIGHT_ORANGE, YELLOW, RESET, GRAY, WHITE, NEON_GREEN, BOLD,
    LIGHT_BLUE, CYAN, LIGHT_GREEN, ORANGE, GREEN, PURPLE, RED
)
...
class InfoCommand(BaseCommand):
    name = "info"
    description = "Get server details (Title, Players, Version, etc.)"
    usage = "info <ip:port>"
    ...
    def run(self, args):
        ...
        try:
            ...
            if plugins:
                plugins_sorted = sorted(plugins, key=lambda x: x.get("name", "").lower())
